...
def print_results(coverage_maps: Dict[str, Any], output_directory: str = '.'):
  ...
  with open(os.path.join(output_dir_path.as_posix(), "coverage.html"), "w") as f:
...
def clone_repos() -> List[str]:
  rules_repos = [
    'https://github.com/returntocorp/semgrep-rules',
    'https://github.com/returntocorp/semgrep-rules-proprietary',
  ]
  repos_path = []

  for repo in rules_repos:
    r_path = Path('/tmp') / f'rules{len(repos_path)}'
    output_dir = r_path.as_posix()
    ...
    p.check_returncode()
    repos_path.append(output_dir)

  return repos_path

def upload_to_s3(content_dir: str):
